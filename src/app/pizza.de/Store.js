...
import {
  asString as getFiltersFromURLAsString,
  asObject as getFiltersFromURLAsObject
} from './lib/getFiltersFromURL'
...
class Store {
  ...
  loadRestaurants = () => {
    getRestaurants().then(this.setLoadedRestaurants)
  }
  ...
  getFilters = () => {
    getFilters().then(this.setFilters)
  }
  ...
  getRestaurantSelected = id => {
    getRestaurant(id).then(this.setRestaurantSelected)
  }
  ...
  get restaurants() {
    ...
    if (
      ...
      filters.categories.indexOf(filteringBy.category) !== -1
    ) {
